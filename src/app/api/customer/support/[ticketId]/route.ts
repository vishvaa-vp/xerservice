...
import { getTicketDetail, SupportServiceError } from '@packages/backend/support';
...
export async function GET(
    ...
    try {
        ...
        return NextResponse.json({ success: true, ...detail }, { status: 200, headers });
    } catch (err: unknown) {
        if (err instanceof SupportServiceError) {
            return NextResponse.json({ error: err.message }, { status: err.statusCode, headers });
        }
        console.error('[GET /api/customer/support/[ticketId]] Error:', err);
        return NextResponse.json(
            { error: err instanceof Error ? err.message : 'Failed to retrieve ticket.' },
            { status: 500, headers }
        );
    }
}
