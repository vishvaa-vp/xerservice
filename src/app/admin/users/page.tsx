...
export default function AdminUsersAndVendorsPage() {
    return <Suspense fallback={<p role="status">Loading accounts…</p>}><AdminUsersAndVendorsContent /></Suspense>;
}

function AdminUsersAndVendorsContent() {
    ...
    // Invitation link state for new user
    ...
    }, [search, roleFilter, statusFilter, shopFilter]);
    ...
    const handleShopFilterChange = (s: string) => {
        setShopFilter(s);
        setCurrentPage(1);
    };
    ...
    }, [roleFilter, statusFilter, shopFilter, adminToken, loadData]);
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        ...
        if (createMethod === 'password' && (!createPassword || createPassword.length < 6)) {
        ...
        try {
            const pwd = createMethod === 'invite'
                ? (Math.random().toString(36).slice(2) + 'Aa1!' + Date.now().toString(36))
                : createPassword;
            ...
            if (data.inviteLink) {
                ...
                setNotification({
                    type: 'success',
                    message: `Account created! Copy the activation link below to invite the user.`,
                });
            } else {
                setNotification({
                    type: 'success',
        ...
        <div style={{ maxWidth: '1360px', margin: '0 auto', padding: '32px 16px' }}>
            ...
            <div style={{
                ...
                <div className="card" style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '12px', fontWeight: '600', color: 'var(--fg-muted)', textTransform: 'uppercase' }}>Total People</div>
                ...
                <div className="card" style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '12px', fontWeight: '600', color: 'var(--fg-muted)', textTransform: 'uppercase' }}>Pending</div>
                        <div style={{ fontSize: '24px', fontWeight: '800', marginTop: '2px', color: '#d97706' }}>
                            {summary.pending ?? 0}
                        </div>
                    </div>
                </div>
                ...
                <div className="card" style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '12px', fontWeight: '600', color: 'var(--fg-muted)', textTransform: 'uppercase' }}>Inactive</div>
                        <div style={{ fontSize: '24px', fontWeight: '800', marginTop: '2px', color: (summary.inactive ?? summary.disabled) > 0 ? '#ef4444' : 'var(--fg)' }}>
                            {summary.inactive ?? summary.disabled}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="card" style={{ padding: '16px 20px', marginBottom: '20px' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        {/* Shop Filter */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ fontSize: '13px', color: 'var(--fg-muted)', fontWeight: '600' }}>Shop:</span>
                            <select
                                value={shopFilter}
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                                        ...
                                        return (
                                            <tr key={u.userId} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.15s' }}>
                                                ...
                                                <td style={{ padding: '14px 16px' }}>
                                                    ...
                                                    ) : u.isPending || u.accountCategory === 'pending' ? (
                                                        <span style={{
                                                            display: 'inline-flex',
                                                            alignItems: 'center',
                                                            gap: '5px',
                ...
                {users.length > 0 && (
                    <div style={{
                        ...
                        <div>
                            Showing <strong>{(currentPage - 1) * pageSize + 1}</strong> to <strong>{Math.min(currentPage * pageSize, users.length)}</strong> of <strong>{users.length}</strong> accounts
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <span style={{ fontSize: '12.5px', fontWeight: '600' }}>
                                Page {currentPage} of {Math.max(1, Math.ceil(users.length / pageSize))}
                            </span>
                            <button
                                onClick={() => setCurrentPage(p => Math.min(Math.max(1, Math.ceil(users.length / pageSize)), p + 1))}
                ...
                <div style={{
                    ...
                    <div className="card" style={{ maxWidth: '520px', width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: '24px' }}>
                        ...
                        <form onSubmit={handleCreateSubmit}>
                            ...
                            <div style={{ marginBottom: '14px' }}>
                                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Credential Setup *</label>
                                <div style={{ display: 'flex', gap: '14px', alignItems: 'center', flexWrap: 'wrap' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', cursor: 'pointer' }}>
                                        ...
                                        />
                                        Generate Activation Link (Recommended)
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', cursor: 'pointer' }}>
                                        <input
                                            type="radio"
                                            name="createMethod"
                                            checked={createMethod === 'password'}
                                            onChange={() => setCreateMethod('password')}
                                        />
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '14px' }}>
                                ...
                                <div>
                                    {createMethod === 'password' ? (
                                        ...
                                        <div style={{ padding: '8px 10px', borderRadius: '8px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', fontSize: '12px', color: 'var(--fg-muted)', height: '100%', display: 'flex', alignItems: 'center' }}>
                                            An expiring activation link will be generated. The user sets their own password.
                                        </div>
                                    )}
                                </div>
                            </div>

                            {/* Show generated invite link if available */}
                            {newInviteLink && (
                                <div style={{ marginBottom: '16px', padding: '12px 14px', borderRadius: '8px', background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.3)' }}>
                    ...
                    <div className="card" style={{ maxWidth: '480px', width: '100%', padding: '24px' }}>
                        ...
                        <form onSubmit={handleEditSubmit}>
                            ...
                            <div style={{ marginBottom: '18px', paddingBottom: '14px', borderBottom: '1px solid var(--border)' }}>
                                <div style={{ fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', color: 'var(--fg-muted)', letterSpacing: '0.05em', marginBottom: '10px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--fg-muted)', marginTop: '4px' }}>
                                        Contact changes update display info. Verified identity requires user OTP authentication.
                                    </div>
                                </div>
                            </div>

                            {/* Section 2: Access & Role */}
                            <div style={{ marginBottom: '18px', paddingBottom: '14px', borderBottom: editRole === 'vendor' ? '1px solid var(--border)' : 'none' }}>
                                <div style={{ fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', color: 'var(--fg-muted)', letterSpacing: '0.05em', marginBottom: '10px' }}>
                                    2. Access & Role
                                </div>
                                <div>
