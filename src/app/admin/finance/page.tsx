...
function AdminFinanceContent() {
    ...
    // Astraplan legacy routing compatibility
    useEffect(() => {
        const view = searchParams.get('view');
        if (view === 'summary' || view === 'overview') {
            router.replace('/admin/earnings');
        } else if (view === 'payouts' || view === 'settlements') {
    ...
    const [reconCategoryFilter, setReconCategoryFilter] = useState<'ALL' | ReconciliationCategory>('ALL');
    const [reconSeverityFilter, setReconSeverityFilter] = useState<'ALL' | ReconciliationSeverity>('ALL');

    // Commercial Audit State
    const [selectedOrderAudit, setSelectedOrderAudit] = useState<OrderPricingExplanation | null>(null);
    ...
    const selectTab = useCallback((tab: FinanceTab) => {
        setActiveTab(tab);
        const view = tab === 'overview' ? 'summary' : tab === 'settlements' ? 'payouts' : tab === 'reconciliation' ? 'reconciliation' : 'shops';
    ...
    useEffect(() => {
        ...
        setActiveTab(
            view === 'shops' ? 'shops' :
            view === 'payouts' || view === 'settlements' ? 'settlements' :
        ...
        try {
            if (tab === 'overview') {
                const [sumRes, reconRes] = await Promise.all([
                    fetch('/api/admin/finance/summary', { headers: { Authorization: `Bearer ${authToken}` } }),
                    fetch('/api/admin/finance/reconciliation', { headers: { Authorization: `Bearer ${authToken}` } }),
                ]);
                if (!sumRes.ok) throw new Error((await sumRes.json()).error || 'Failed to load summary');
                setSummary(await sumRes.json());
                if (reconRes.ok) {
                    const rData = await reconRes.json();
                    setReconciliationReport(rData.report || null);
                }
            } else if (tab === 'shops') {
                ...
                if (!res.ok) throw new Error((await res.json()).error || 'Failed to load reconciliation report');
    ...
    // Commercial Order Pricing Audit Handler
    const handleOpenOrderAudit = async (orderId: string) => {
        const authToken = await getValidToken();
        if (!authToken) return;
        setAuditLoading(true);
        setError(null);
        try {
            const res = await fetch(`/api/admin/finance/orders/${orderId}/pricing-audit`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            if (!res.ok) throw new Error((await res.json()).error || 'Failed to load pricing audit');
            const data = await res.json();
            setSelectedOrderAudit(data.audit || null);
        } catch (err: any) {
            setError(err.message || 'Error auditing order pricing');
        } finally {
    ...
    return (
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '32px 16px' }}>
            ...
            <div role="tablist" aria-label="Payment views" style={{ display: 'flex', overflowX: 'auto', borderBottom: '1px solid var(--border)', marginBottom: '28px', gap: '4px' }}>
                {financeTabs.map(tab => {
                    const view = tab.id === 'overview' ? 'summary' : tab.id === 'settlements' ? 'payouts' : tab.id === 'reconciliation' ? 'reconciliation' : 'shops';
                    const isRecon = tab.id === 'reconciliation';
                    const issueCount = reconciliationReport?.summary.totalIssues || 0;
                    return (
                        <Link
                            key={tab.id}
                            href={`/admin/finance?view=${view}`}
                            role="tab"
            ...
            {activeTab === 'overview' && summary && (
                <div>
                    {reconciliationReport && reconciliationReport.summary.totalIssues > 0 && (
                        <div style={{ marginBottom: '24px', padding: '16px', borderRadius: '14px', background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <AlertTriangle size={24} style={{ color: '#dc2626' }} />
                                <div>
                                    <div style={{ fontWeight: '800', color: '#b91c1c', fontSize: '15px' }}>
                                        {reconciliationReport.summary.totalIssues} Financial Reconciliation Issue{reconciliationReport.summary.totalIssues > 1 ? 's' : ''} Detected
                                    </div>
                                    <div style={{ fontSize: '13px', color: 'var(--fg-muted)', marginTop: '2px' }}>
                                        {reconciliationReport.summary.unallocatedOrdersCount > 0 && `${reconciliationReport.summary.unallocatedOrdersCount} unallocated order(s) · `}
                                        {reconciliationReport.summary.unresolvedRefundsCount > 0 && `${reconciliationReport.summary.unresolvedRefundsCount} unresolved refund(s) · `}
                                        {reconciliationReport.summary.settlementMismatchesCount > 0 && `${reconciliationReport.summary.settlementMismatchesCount} settlement mismatch(es)`}
                                    </div>
                                </div>
                            </div>
                            <button onClick={() => selectTab('reconciliation')} className="btn btn-secondary btn-sm" style={{ borderColor: '#dc2626', color: '#b91c1c' }}>
                                Open Reconciliation Inbox <ArrowRight size={14} />
                            </button>
                        </div>
                    )}
                ...
                <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <h2 style={{ fontSize: '18px', fontWeight: '800' }}>Financial Reconciliation Inbox</h2>
                            </div>
                            <p style={{ color: 'var(--fg-muted)', fontSize: '13px', marginTop: '2px' }}>
                                Surface and triage unallocated paid orders, unresolved refunds, and settlement mismatches.
                            </p>
                        </div>
                        <button
                            onClick={async () => {
                                const curToken = await getValidToken();
                                if (curToken) loadTabData('reconciliation', curToken);
                            }}
                            className="btn btn-secondary btn-sm"
                            style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                            disabled={loading}
                    ...
                    {reconciliationReport ? (
                        <>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                                ...
                                <div
                                    ...
                                    style={{
                                        padding: '20px',
                                        borderLeft: reconciliationReport.summary.totalIssues === 0
                                            ? '4px solid #16a34a'
                                            : reconciliationReport.summary.criticalIssues > 0
                                            ? '4px solid #dc2626'
                                            : '4px solid #f59e0b',
                                    }}
                                >
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <span style={{ fontSize: '13px', color: 'var(--fg-muted)', fontWeight: '700' }}>Reconciliation Health</span>
                                        {reconciliationReport.summary.totalIssues === 0 ? (
                                            <CheckCircle2 size={18} style={{ color: '#16a34a' }} />
                                        ) : (
                                            <AlertCircle size={18} style={{ color: reconciliationReport.summary.criticalIssues > 0 ? '#dc2626' : '#f59e0b' }} />
                                        )}
                                    </div>
                                    <div style={{ fontSize: '24px', fontWeight: '900', marginTop: '8px', color: reconciliationReport.summary.totalIssues === 0 ? '#16a34a' : 'var(--fg)' }}>
                                        {reconciliationReport.summary.totalIssues === 0 ? 'All Reconciled' : `${reconciliationReport.summary.totalIssues} Action Items`}
                                    </div>
                                    <div style={{ fontSize: '12px', color: 'var(--fg-subtle)', marginTop: '4px' }}>
                                        {reconciliationReport.summary.totalIssues === 0
                                            ? '100% balanced across accounts'
                                            : `${reconciliationReport.summary.criticalIssues} Critical · ${reconciliationReport.summary.highIssues} High`}
                                    </div>
                                </div>

                                {/* Card 2: Unallocated Orders */}
                                <div className="card" style={{ padding: '20px' }}>
                                    <div style={{ fontSize: '13px', color: 'var(--fg-muted)', fontWeight: '700' }}>Unallocated Paid Orders</div>
                                    <div style={{ fontSize: '24px', fontWeight: '900', marginTop: '8px', color: reconciliationReport.summary.unallocatedOrdersCount > 0 ? '#f59e0b' : 'var(--fg)' }}>
                                        {reconciliationReport.summary.unallocatedOrdersCount}
                                    </div>
                                    <div style={{ fontSize: '12px', color: 'var(--fg-subtle)', marginTop: '4px' }}>
                                        ₹{reconciliationReport.summary.unallocatedAmount.toFixed(2)} unallocated volume
                                    </div>
                                </div>

                                {/* Card 3: Unresolved Refunds */}
                                <div className="card" style={{ padding: '20px' }}>
                                    <div style={{ fontSize: '13px', color: 'var(--fg-muted)', fontWeight: '700' }}>Unresolved Refunds</div>
                                    <div style={{ fontSize: '24px', fontWeight: '900', marginTop: '8px', color: reconciliationReport.summary.unresolvedRefundsCount > 0 ? '#dc2626' : 'var(--fg)' }}>
                                        {reconciliationReport.summary.unresolvedRefundsCount}
                                    </div>
                                    <div style={{ fontSize: '12px', color: 'var(--fg-subtle)', marginTop: '4px' }}>
                                        ₹{reconciliationReport.summary.unresolvedRefundAmount.toFixed(2)} pending refund
                                    </div>
                                </div>

                                {/* Card 4: Settlement Mismatches */}
                                <div className="card" style={{ padding: '20px' }}>
                                    <div style={{ fontSize: '13px', color: 'var(--fg-muted)', fontWeight: '700' }}>Settlement Discrepancies</div>
                                    <div style={{ fontSize: '24px', fontWeight: '900', marginTop: '8px', color: reconciliationReport.summary.settlementMismatchesCount > 0 ? '#dc2626' : 'var(--fg)' }}>
                                        {reconciliationReport.summary.settlementMismatchesCount}
                                    </div>
                                    <div style={{ fontSize: '12px', color: 'var(--fg-subtle)', marginTop: '4px' }}>
                                        Batch & ledger balance discrepancies
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                    {[
                                        { id: 'ALL', label: `All (${reconciliationReport.summary.totalIssues})` },
                                        { id: 'UNALLOCATED_ORDER', label: `Unallocated Orders (${reconciliationReport.summary.unallocatedOrdersCount})` },
                                        { id: 'UNRESOLVED_REFUND', label: `Unresolved Refunds (${reconciliationReport.summary.unresolvedRefundsCount})` },
                                        { id: 'SETTLEMENT_MISMATCH', label: `Settlements (${reconciliationReport.summary.settlementMismatchesCount})` },
                                    ].map(pill => (
                                        <button
                                            key={pill.id}
                                            onClick={() => setReconCategoryFilter(pill.id as any)}
                                            style={{
                                                padding: '6px 12px',
                                                borderRadius: '20px',
                                                fontSize: '12px',
                                                fontWeight: reconCategoryFilter === pill.id ? '700' : '500',
                                                background: reconCategoryFilter === pill.id ? 'var(--fg)' : 'var(--bg-secondary)',
                                                color: reconCategoryFilter === pill.id ? 'var(--bg)' : 'var(--fg-muted)',
                                ...
                                {/* Severity Filter */}
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span style={{ fontSize: '12px', color: 'var(--fg-muted)', fontWeight: '600' }}>Severity:</span>
                                    <select
                                        className="input"
                                        style={{ padding: '5px 10px', fontSize: '12px', width: 'auto' }}
                                        value={reconSeverityFilter}
                                        onChange={(e) => setReconSeverityFilter(e.target.value as any)}
                                    >
                                        <option value="ALL">All Severities</option>
                                        <option value="CRITICAL">Critical Only ({reconciliationReport.summary.criticalIssues})</option>
                                        <option value="HIGH">High ({reconciliationReport.summary.highIssues})</option>
                                        <option value="MEDIUM">Medium ({reconciliationReport.summary.mediumIssues})</option>
                                        <option value="LOW">Low ({reconciliationReport.summary.lowIssues})</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            {(() => {
                                const filteredItems = reconciliationReport.items.filter(item => {
                                    if (reconCategoryFilter !== 'ALL' && item.category !== reconCategoryFilter) return false;
                                    if (reconSeverityFilter !== 'ALL' && item.severity !== reconSeverityFilter) return false;
                                    return true;
                                });

                                if (filteredItems.length === 0) {
                                    if (reconciliationReport.summary.totalIssues === 0) {
                                        return (
                                            <div className="card" style={{ textAlign: 'center', padding: '56px 24px' }}>
                                                <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(22, 163, 74, 0.1)', color: '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                                                    <CheckCheck size={28} />
                                                </div>
                                                <h3 style={{ fontSize: '18px', fontWeight: '800', marginBottom: '6px' }}>All Financial Accounts Balanced</h3>
                                                <p style={{ color: 'var(--fg-muted)', fontSize: '14px', maxWidth: '520px', margin: '0 auto' }}>
                                                    Every customer payment has a corresponding ledger row, shop commissions are properly allocated, customer refunds are completed, and all settlement batches match item aggregates.
                                                </p>
                                            </div>
                                        );
                                    }
                                    ...
                                    return (
                                        <div className="card" style={{ textAlign: 'center', padding: '40px 24px', color: 'var(--fg-muted)' }}>
                                            <p style={{ fontSize: '14px', marginBottom: '12px' }}>No reconciliation items match your active filters.</p>
                                            <button
                                                onClick={() => {
                                                    setReconCategoryFilter('ALL');
                                                    setReconSeverityFilter('ALL');
                                                }}
                                                className="btn btn-secondary btn-sm"
                                            >
                                                Reset Filters
                                ...
                                return (
                                    <div className="card" style={{ overflowX: 'auto', padding: 0 }}>
                                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
                                            <thead>
                                                <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--fg-muted)' }}>
                                                    <th style={{ padding: '12px 16px', fontWeight: '700', width: '100px' }}>Severity</th>
                                                    <th style={{ padding: '12px 16px', fontWeight: '700' }}>Discrepancy Details & Diagnosis</th>
                                                    <th style={{ padding: '12px 16px', fontWeight: '700', width: '180px' }}>Shop / Entity</th>
                                                    <th style={{ padding: '12px 16px', fontWeight: '700', width: '110px' }}>Impact</th>
                                                    <th style={{ padding: '12px 16px', fontWeight: '700', width: '160px' }}>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                    ...
                                                    return (
                                                        <tr key={item.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                                            ...
                                                            <td style={{ padding: '14px 16px', verticalAlign: 'top' }}>
                                                                ...
                                                                <div style={{ fontSize: '10px', color: 'var(--fg-subtle)', marginTop: '4px' }}>
                                                            ...
                                                            {/* Details & Diagnosis */}
                                                            <td style={{ padding: '14px 16px', verticalAlign: 'top' }}>
                                                                <div style={{ fontWeight: '700', color: 'var(--fg)', fontSize: '14px', marginBottom: '2px' }}>
                                                                    {item.title}
                                                                </div>
                                                                <div style={{ color: 'var(--fg-muted)', fontSize: '13px', lineHeight: '1.4' }}>
                                                                    {item.description}
                                                                </div>
                                                                <div style={{ color: 'var(--fg-subtle)', fontSize: '11px', marginTop: '6px' }}>
                                                                    <strong>Fix:</strong> {item.action.suggestedFix}
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td style={{ padding: '14px 16px', verticalAlign: 'top' }}>
                                                                <div style={{ fontWeight: '600', fontSize: '13px' }}>
                                                                    {item.shopName || 'Platform'}
                                                                </div>
                                                                {item.orderId && (
                                                                    <div style={{ fontSize: '11px', color: 'var(--fg-subtle)', fontFamily: 'monospace', marginTop: '2px' }}>
                                                                        Ord: #{item.orderId.slice(0, 8)}
                                                                    </div>
                                                                )}
                                                                {item.batchId && (
                                                                    <div style={{ fontSize: '11px', color: 'var(--fg-subtle)', fontFamily: 'monospace', marginTop: '2px' }}>
                                                                        Batch: {item.settlementNumber || item.batchId.slice(0, 8)}
                                                                    </div>
                                                                )}
                                                            </td>

                                                            {/* Impact */}
                                                            <td style={{ padding: '14px 16px', verticalAlign: 'top', fontWeight: '800', color: 'var(--fg)' }}>
                                                            ...
                                                            <td style={{ padding: '14px 16px', verticalAlign: 'top' }}>
                                                                <button
                                                                    onClick={() => {
                                                                        if (item.action.deepLink.includes('tab=shops')) {
                                                                            selectTab('shops');
                                                                        } else if (item.action.deepLink.includes('tab=settlements')) {
                                                                            selectTab('settlements');
                                                                        } else {
                                                                            router.push(item.action.deepLink);
                                                                        }
                                                                    }}
                                                                    className="btn btn-accent btn-sm"
                                                                    style={{ fontSize: '12px', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '4px' }}
                                                                >
                                                                    {item.action.label}
                                                                    <ArrowRight size={12} />
                                                                </button>
                        ...
                        <div className="card" style={{ textAlign: 'center', padding: '40px' }}>
                            <RefreshCw className="animate-spin" size={24} style={{ margin: '0 auto 12px', color: 'var(--accent)' }} />
                            <p style={{ color: 'var(--fg-muted)', fontSize: '14px' }}>Loading reconciliation audit data...</p>
                        </div>
                    )}
                </div>
            )}
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--fg-muted)' }}>
                                        ...
                                        <th style={{ padding: '10px 12px', fontWeight: '700' }}>Audit</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {(selectedShopDetails.ledgers || []).map((l: any) => (
                                        <tr key={l.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                            ...
                                            <td style={{ padding: '10px 12px' }}>
                                                <button
                                                    onClick={() => handleOpenOrderAudit(l.orderId || l.order_id)}
                                                    className="btn btn-secondary btn-sm"
            ...
            {/* MODAL: COMMERCIAL PRICING AUDIT */}
            {selectedOrderAudit && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 110 }}>
                    <div className="card" style={{ width: '100%', maxWidth: '820px', maxHeight: '90vh', overflowY: 'auto', padding: '28px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <div>
                                <h3 style={{ fontSize: '20px', fontWeight: '800' }}>Commercial Pricing Audit</h3>
                                <p style={{ fontSize: '13px', color: 'var(--fg-muted)' }}>
                                    Order #{selectedOrderAudit.orderId.slice(0, 8)} · {selectedOrderAudit.shopName}
                                </p>
                            </div>
                            <button onClick={() => setSelectedOrderAudit(null)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                                <X size={20} />
                            </button>
                        </div>

                        {/* Narrative */}
                        <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', marginBottom: '20px', fontSize: '13px', lineHeight: '1.5' }}>
                            <strong>Calculation Explanation:</strong> {selectedOrderAudit.humanNarrative}
                        </div>

                        {/* Commercial Change Warning Banner if Rates / Rules have changed */}
                        {selectedOrderAudit.hasCommercialChangesSinceCheckout && (
                            <div style={{ background: 'rgba(234, 179, 8, 0.1)', border: '1px solid rgba(234, 179, 8, 0.3)', borderRadius: '12px', padding: '14px', marginBottom: '20px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                                <AlertTriangle size={18} style={{ color: '#ca8a04', flexShrink: 0, marginTop: '2px' }} />
                                <div style={{ fontSize: '13px', color: 'var(--fg)' }}>
                                    <div style={{ fontWeight: '700', color: '#a16207' }}>Commercial Rates Updated After Order Placement</div>
                                    <div style={{ color: 'var(--fg-muted)', marginTop: '2px' }}>
                                        {selectedOrderAudit.commissionAudit.commissionDeltaExplanation}
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <h4 style={{ fontSize: '14px', fontWeight: '700', marginBottom: '10px' }}>Document Pricing Waterfall</h4>
                        <div className="card" style={{ overflowX: 'auto', padding: 0, marginBottom: '20px' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--fg-muted)' }}>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Document</th>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Pages & Mode</th>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Sheets Consumed</th>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Checkout Rate</th>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Current Rate</th>
                                        <th style={{ padding: '10px 14px', fontWeight: '700' }}>Line Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {selectedOrderAudit.fileBreakdowns.map((f) => (
                                        <tr key={f.orderFileId} style={{ borderBottom: '1px solid var(--border)' }}>
                                            <td style={{ padding: '12px 14px', fontWeight: '600' }}>{f.filename}</td>
                                            <td style={{ padding: '12px 14px' }}>
                                                <div>{f.printablePages} pages · {f.colourMode}</div>
                                                <div style={{ fontSize: '11px', color: 'var(--fg-subtle)' }}>
                                                    {f.isDuplex ? 'Double-sided' : 'Single-sided'} ({f.paperSize})
                                                </div>
                                            </td>
                                            <td style={{ padding: '12px 14px', fontWeight: '700' }}>
                                                {f.physicalSheets} sheets
                                                <div style={{ fontSize: '11px', color: 'var(--fg-subtle)', fontWeight: 'normal' }}>
                                                    {f.copies} copy
                                                </div>
                                            </td>
                                            <td style={{ padding: '12px 14px', fontWeight: '700' }}>₹{f.appliedUnitPrice.toFixed(2)}/sheet</td>
                                            <td style={{ padding: '12px 14px' }}>
                                                {f.currentShopRate !== null ? (
                                                    <span style={{ color: f.rateMatchedCurrent ? 'inherit' : '#ea580c', fontWeight: f.rateMatchedCurrent ? 'normal' : '700' }}>
                                                        ₹{f.currentShopRate.toFixed(2)}/sheet
                                                        {!f.rateMatchedCurrent && (
                                                            <div style={{ fontSize: '10px', color: '#ea580c' }}>Rate changed</div>
                                                        )}
                                                    </span>
                                                ) : (
                                                    <span style={{ color: 'var(--fg-subtle)' }}>Not in catalog</span>
                                                )}
                                            </td>
                        ...
                        {/* Commercial Revenue Split Breakdown */}
                        <h4 style={{ fontSize: '14px', fontWeight: '700', marginBottom: '10px' }}>Commercial Revenue Split</h4>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                            <div className="card" style={{ padding: '14px' }}>
                                <div style={{ fontSize: '12px', color: 'var(--fg-muted)' }}>Customer Gross Payment</div>
                                <div style={{ fontSize: '20px', fontWeight: '800', marginTop: '4px' }}>₹{selectedOrderAudit.totalAmount.toFixed(2)}</div>
                            </div>
                            <div className="card" style={{ padding: '14px' }}>
                                <div style={{ fontSize: '12px', color: 'var(--fg-muted)' }}>
                                    XerService Fee ({selectedOrderAudit.commissionAudit.appliedPercentage !== null ? `${selectedOrderAudit.commissionAudit.appliedPercentage.toFixed(2)}%` : '—'})
                                </div>
                                <div style={{ fontSize: '20px', fontWeight: '800', marginTop: '4px', color: 'var(--accent)' }}>
                                    {selectedOrderAudit.commissionAudit.platformCommissionAmount !== null ? `₹{selectedOrderAudit.commissionAudit.platformCommissionAmount.toFixed(2)}` : '—'}
                                </div>
                            </div>
                            <div className="card" style={{ padding: '14px' }}>
                                <div style={{ fontSize: '12px', color: 'var(--fg-muted)' }}>Vendor Net Payout</div>
                                <div style={{ fontSize: '20px', fontWeight: '800', marginTop: '4px', color: '#16a34a' }}>
                                    {selectedOrderAudit.commissionAudit.vendorNetAmount !== null ? `₹{selectedOrderAudit.commissionAudit.vendorNetAmount.toFixed(2)}` : '—'}
                                </div>
                            </div>
                        </div>

                        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                            <button onClick={() => setSelectedOrderAudit(null)} className="btn btn-secondary">
                                Close Audit
                            </button>
                        </div>
                    </div>
